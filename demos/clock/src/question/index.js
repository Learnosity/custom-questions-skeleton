...
export default class ClockQuestion {
  ...
  render() {
    ...
    return Promise.all([
      ...
      this.clock = new Clock(el.querySelector("svg"), { question, response });
  ...
  handleEvents() {
    ...
    const dragHandler = drag()
      ...
      .on("drag", function (event) {
        let rad = Math.atan2(event.y - CLOCK_CENTER, event.x - CLOCK_CENTER);

        let degrees = rad / (Math.PI / 180);

        if (degrees < 0) degrees = 360 + degrees;

        // it thinks the hand belongs to other clock.
        /**
         * eg: hand is in first clock
         * but the question logged pertains to the second clock
         */
        const hand = this;
        d3.select(hand)
      ...
      .on("end", () => {
        let responseObject = {
          ...
          hourHandAngle: roundAngle(clock.hourHand.attr("data-angle")),
        };

        events.trigger("changed", responseObject);

        clearValidationUI([face, feedback]);
      });
    ...
    events.on("validate", (options) => {
      ...
      showValidationUI(result, [face, feedback]);
