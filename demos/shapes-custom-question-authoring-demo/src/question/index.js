...
export default class Question {
             ...
             if (init.state === 'resume') {
                ...
                if(init.response) {
                    ...
                    shapeButtons.forEach((shapeButton) => {
                        if(shapeButton.getAttribute('data-index') === init.response) {
    ...
    render() {
        ...
        return Promise.all([
            ...
            question.shapes && question.shapes.forEach((shape, index) => {
                const { kind, size, color } = shape;
                const shapeWapper = document.createElement('div');
                shapeWapper.setAttribute('class', 'shape-wrapper flex-column');
                ...
                if(kind === "octagon") {
                    shapeBtn.setAttribute('style', `width:${pxValue}px;background:${color};`)
                }
                if(kind === "triangle") {
                    shapeBtn.setAttribute('style', `width:${pxValue+10}px;background:${color};`)
                }
                if(kind === "square" || kind === "circle") {
                    shapeBtn.setAttribute('style', `width: ${pxValue}px; height: ${pxValue}px; background: ${color};`)
                }
                if(kind === "hexagon") {
                    shapeBtn.setAttribute('style', `height: ${pxValue}px; background: ${color};`)
                }
                shapeWapper.appendChild(shapeBtn);
                if(question.display_shape_names) {
                    const p = document.createElement('p');
                    p.innerText = kind;
                    shapeWapper.appendChild(p)
                }
                el.querySelector('.shapes').appendChild(shapeWapper)
            })
        ...
        events.on('validate', options => {
            ...
            if (!facade.isValid() && options.showCorrectAnswers) {
                ...
                * So, for a hypothetical question with 3 correct answers, you might pass the following array into setAnswers():
                ...
                this.suggestedAnswersList.setAnswers([{index: Number(question.validation.valid_response.value), label: question.shapes[Number(question.validation.valid_response.value)].kind.toUpperCase()}]);
            }
